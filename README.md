...
* Запись выглядит так: адрес, метка языка интерфейса в скобках — *(ru)*, *(en)* или *(multi)*, — тире и описание одной строкой.
* Добавляйте курс в тот раздел, к которому он относится.
...
* https://2019.wattenberger.com/blog/css-cascade (en) – Объяснение CSS каскада с интерактивом
...
* http://tomorrowcorporation.com/humanresourcemachine (en) – Паззл-игра введение в ассемблер.
* http://tomorrowcorporation.com/7billionhumans (en) – Продолжение Human Resource Machine, мультипроцессорный ассемблер.
* https://www.zachtronics.com/tis-100/ (en) – Параллельный ассемблер для вымышленного компьютера.
* https://www.zachtronics.com/shenzhen-io/ (en) – Игра, где нужно разводить платы и программировать их на ассемблере.
...
* https://commonmark.org/help/tutorial/index.html (en) – интерактивное руководство по Markdown.

### Хакинг

* https://www.hacksplaining.com/lessons (en) – через интерактивные уроки наглядно объясняются основные веб-уязвимости. Каждый урок включает геймифицированную демонстрацию атак и стратегии их устранения.
...
* https://exercism.org/ (en) – Специально подобранные наборы задач (learning tracks) для практики на разных языках. Задачи решаются offline. Решение загружается на сайт и проверяется. Можно бесплатно получить обратную связь от ментора или сообщества.
...
* https://koddo.ru/trainers/sql (ru) – Тренажёр SQL: 19 задач с автопроверкой прямо в браузере (встроенный PostgreSQL), без регистрации.
...
* https://lost-at-sql.therobinlord.com/ (en) – Вы капитан тонующей подводной лодки. Удачи! Кстати, тут есть не только основной сюжет, но и доп.задания без подсказок, а также таблица лидеров.

* https://sql-island.informatik.uni-kl.de/?lang=en (multi) – Вы оказываетесь на острове после авиакатастрофы и должны использовать свои навыки SQL, чтобы найти способ выбраться с него.

* https://sqlpd.com/ (en) – Подтягиваем базовый синтаксис и раскрываем преступления вместе с этой игрой. Данные для анализа вам предоставляет этичный хакер.

* https://datalemur.com/sql-game (en) – Вы на том самом острове в качестве аналитика. И вам нужно рассчитать, например, количество порций, которое создаст «нужное напряжение» и прокормит лишь 90% игроков. Всего 9 уровней.
...
* https://web.mit.edu/mprat/Public/web/Terminus/Web/main.html (en) – браузерная игра от MIT, где игрок изучает команды, перемещаясь по виртуальному миру и обучаясь в Академии Волшебства.
...
* http://ndpsoftware.com/git-cheatsheet.html (en) – Интерактивная шпаргалка по командам Git
...
* https://klava.org/delta/ (ru) – Это клавиатурный тренажёр (а так же сервис для изучения иностранных языков и скорочтения). Без регистрации, с визуальными подсказками, таймером, возможно добавлять собственные тексты. Показывает скорость печать и количество ошибок. Помимо русского, английского, украинского и эсперанто есть так же языки программирования:  C, C++, Go, JavaScript, Pascal, PHP, Python, SQL, XML/XSLT.
